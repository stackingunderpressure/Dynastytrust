...
export function legacyRecoveryPackageText(p: LegacyRecoveryPackageLike): string {
  const lines = [
    ...
    ...(p.identityPubkeyHex && p.lockedFastShareSigB64
      ? [
          `# ---------------------------------------------------------------`,
          `# IF DYNASTYTRUST IS STILL RUNNING -- a hardware-wallet-friendly`,
          `# shortcut that skips typing your seed phrase into anything`,
          `# ---------------------------------------------------------------`,
          `# Paste your account xpub into DynastyTrust's "Retrieve a`,
          `# descriptor" page. It finds this exact share automatically and`,
          `# unlocks it with a SIGNATURE from your key (a hardware wallet's`,
          `# own "Sign Message" feature works) instead of the fast-path`,
          `# fields above. This identity pubkey confirms the match:`,
