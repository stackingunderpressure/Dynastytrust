...
const inp: React.CSSProperties = {
  ...
  border: '1px solid #1E1E30', borderRadius: 8, color: '#E8E4D8',
...
const lbl: React.CSSProperties = {
  fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', color: '#5A5570',
  textTransform: 'uppercase', marginBottom: 5, display: 'block',
};
const goldBtn: React.CSSProperties = {
  padding: '10px 20px', background: '#C9A84C', border: 'none', borderRadius: 8,
  color: '#07070F', fontWeight: 700, fontSize: 14, fontFamily: 'DM Sans, sans-serif', cursor: 'pointer',
};
const ghostBtn: React.CSSProperties = {
  padding: '9px 16px', background: 'none', border: '1px solid #1E1E30',
  borderRadius: 8, color: '#9994A8', fontSize: 13, fontFamily: 'DM Sans, sans-serif', cursor: 'pointer',
};
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 5 }}>
        {words.map((w, i) => (
          <div key={i} style={{
            ...
            <span style={{ fontSize: 10, color: '#5A5570', minWidth: 16, flexShrink: 0 }}>{i + 1}</span>
            <span style={{
              fontSize: 12, fontFamily: 'IBM Plex Mono, monospace',
              color: visible ? '#E8E4D8' : 'transparent',
              textShadow: visible ? 'none' : '0 0 8px #5A5570',
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 6 }}>
        {all.map(p => (
          <button key={p} onClick={() => { onChange(p); setShowCustom(false); }} style={{
            ...
            borderColor: value === p ? '#C9A84C' : '#1E1E30',
            color: value === p ? '#C9A84C' : '#9994A8',
      ...
      {showCustom && (
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button style={ghostBtn} onClick={() => {
            if (custom.trim()) { onChange(custom.trim()); setShowCustom(false); }
          }}>Set</button>
        </div>
      )}
    </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none',
            color: '#5A5570', fontSize: 18, cursor: 'pointer' }}>x</button>
        </div>
        {children}
      </div>
    </div>
  ...
  return (
    <Modal title="Quick test key" onClose={onClose}>
      <div style={{ padding: '10px 14px', background: '#0A1A14',
        border: '1px solid #52C47A44', borderRadius: 8, marginBottom: 18 }}>
        <p style={{ fontSize: 13, color: '#52C47A', margin: 0 }}>
      ...
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button type="submit" style={{ ...goldBtn, background: '#52C47A' }}>
            Generate instantly
          </button>
        </div>
      </form>
...
function TestKeyCreated({ keyData, mnemonic, onClose }: {
  ...
  return (
    <Modal title="Key generated" onClose={onClose} wide>
      <div style={{ padding: '10px 14px', background: '#0A1A14',
        border: '1px solid #52C47A44', borderRadius: 8, marginBottom: 16 }}>
        <p style={{ fontSize: 13, color: '#52C47A', margin: 0 }}>
          Created {keyData.label} for {keyData.persona}. Mnemonic saved locally.
        </p>
      </div>
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 13, color: '#5A5570', marginBottom: 10 }}>
  ...
  return (
    <Modal title="Secure key" onClose={onClose}>
      ...
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {err && <p style={{ color: '#E05C5C', fontSize: 13, margin: 0 }}>{err}</p>}
        <div style={{ display: 'flex', gap: 10 }}>
          <button type="button" style={ghostBtn} onClick={onClose}>Cancel</button>
...
function BackupFlow({ keyData, mnemonic, onDone }: {
  ...
  function verify() {
    ...
    if (wrong.length) {
      setErr('Word(s) ' + wrong.map(p => '#' + (p + 1)).join(', ') + ' incorrect.');
      return;
    }
  ...
  if (step === 'show') return (
    <Modal title="Write down your recovery phrase" onClose={() => {}} wide>
      <div style={{ padding: '10px 14px', background: '#1A0A0A', border: '1px solid #3A1A1A',
        ...
        <p style={{ fontSize: 13, color: '#E05C5C', margin: 0 }}>
          Write all 24 words on paper. Never store digitally. Shown once only.
        </p>
      </div>
      <WordGrid words={words} />
      <label style={{ display: 'flex', gap: 10, alignItems: 'center', cursor: 'pointer',
        marginTop: 16, padding: 12, background: '#141422', borderRadius: 8 }}>
        <input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)} />
        <span style={{ fontSize: 13, color: '#9994A8' }}>I have written all 24 words in order.</span>
      </label>
  ...
  return (
    <Modal title="Verify your backup" onClose={() => {}}>
      ...
      {err && <p style={{ color: '#E05C5C', fontSize: 13 }}>{err}</p>}
...
function RevealModal({ keyData, onClose, onBackedUp }: {
  ...
  if (doBackup && mnemonic) {
    return <BackupFlow keyData={keyData} mnemonic={mnemonic}
      onDone={() => { onBackedUp(); onClose(); }} />;
  }
    ...
    <Modal title="Recovery phrase" onClose={onClose} wide>
      {!mnemonic ? (
        <form onSubmit={unlock} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <p style={{ fontSize: 13, color: '#5A5570' }}>
            Enter password for {keyData.label}.
          </p>
          <div>
        ...
        <>
          {isTest ? (
            <div style={{ padding: '10px 14px', background: '#0A1400',
              border: '1px solid #52C47A44', borderRadius: 8, marginBottom: 14 }}>
              <p style={{ fontSize: 12, color: '#52C47A', margin: 0 }}>
            ...
            <div style={{ padding: '10px 14px', background: '#1A0A0A', border: '1px solid #3A1A1A',
              ...
              <p style={{ fontSize: 12, color: '#E05C5C', margin: 0 }}>Keep this private. Close when done.</p>
            </div>
          )}
          <WordGrid words={mnemonic.split(' ')} />
...
function SecureUpgradeModal({ keyData, onDone, onClose }: {
  ...
  return (
    <Modal title="Secure this key" onClose={onClose}>
      <p style={{ fontSize: 13, color: '#5A5570', marginBottom: 18, lineHeight: 1.5 }}>
        Set a password to encrypt the mnemonic for {keyData.label}. The plaintext mnemonic will be deleted.
      </p>
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div>
...
function ImportModal({ onDone, onClose }: { onDone: () => void; onClose: () => void }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    try {
      importXpub({ label: label.trim() || persona, persona, network, xpub: xpub.trim(), derivationPath: path.trim() });
      onDone();
    } catch (e) {
...
function KeyRow({ k, accentColor, onDetail, onReveal }: {
  k: LocalKey; accentColor: string; onDetail: () => void; onReveal: () => void;
}) {
  const isTest = !!k.testMnemonic;
  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 14,
      background: '#0F0F1A', border: '1px solid #1E1E30', borderRadius: 12,
      padding: '14px 18px', cursor: 'pointer',
    }} onClick={onDetail}>
      <div style={{
        width: 40, height: 40, borderRadius: 9, flexShrink: 0,
        background: isTest ? '#52C47A18' : '#4A90D918',
        display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18,
      }}>
        {k.origin === 'software' ? (isTest ? 'T' : 'S') : 'H'}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 15, fontWeight: 600, color: '#E8E4D8' }}>{k.label}</span>
          {isTest && (
            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 4,
              background: '#52C47A22', color: '#52C47A' }}>TEST</span>
          )}
          {!k.backedUp && k.origin === 'software' && (
            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 4,
              background: '#E0905022', color: '#E09050' }}>NOT BACKED UP</span>
          )}
          {k.backedUp && (
            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 4,
              background: '#52C47A22', color: '#52C47A' }}>BACKED UP</span>
          )}
        </div>
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 11, color: accentColor }}>{k.persona}</span>
          <span style={{ fontSize: 11, fontFamily: 'IBM Plex Mono, monospace', color: '#5A5570' }}>{k.fingerprint}</span>
          <span style={{ fontSize: 11, color: k.network === 'mainnet' ? '#C9A84C' : '#52C47A' }}>
            {k.network.toUpperCase()}
          </span>
        </div>
      </div>
...
function DetailModal({ k, onClose, onReveal, onSecure, onArchive, onDelete }: {
  ...
  const keyType = k.testMnemonic ? 'Test key (plaintext mnemonic)'
    : k.origin === 'imported_xpub' ? 'Imported xpub'
    ...
    <Modal title={k.label} onClose={onClose} wide>
      <div style={{ background: '#0A0A14', borderRadius: 10, overflow: 'hidden', marginBottom: 14 }}>
        {[
          ...
          <div key={label} style={{ display: 'flex', justifyContent: 'space-between',
            alignItems: 'center', padding: '9px 14px', borderBottom: '1px solid #1E1E30' }}>
            <span style={{ fontSize: 12, color: '#5A5570' }}>{label}</span>
            <span style={{ fontSize: 13, color: '#E8E4D8' }}>{value}</span>
          </div>
        ))}
      </div>
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ background: '#0A0A14', borderRadius: 8, padding: '10px 12px',
          fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: '#9994A8',
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        {k.testMnemonic && (
          <button style={{ ...ghostBtn, fontSize: 13, color: '#C9A84C', borderColor: '#8B6914' }}
            onClick={onSecure}>
            Secure key
          </button>
        )}
        ...
        <button style={{ ...ghostBtn, fontSize: 13, color: '#E05C5C', borderColor: '#3A1A1A', marginLeft: 'auto' }}
  ...
  return (
    <div style={{ fontFamily: 'DM Sans, sans-serif' }}>
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
        <button style={{ ...goldBtn, background: '#52C47A', fontSize: 14 }}
          onClick={() => setModal({ type: 'quick' })}>
          Quick test key
        </button>
        <button style={{ ...ghostBtn, borderColor: '#8B6914', color: '#C9A84C' }}
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
          {personas.map(p => (
            <button key={p} onClick={() => setFilter(p)} style={{
              ...ghostBtn, padding: '5px 12px', fontSize: 12,
              borderColor: personaFilter === p ? (p === 'all' ? '#C9A84C' : personaColors[p] ?? '#C9A84C') : '#1E1E30',
              color: personaFilter === p ? (p === 'all' ? '#C9A84C' : personaColors[p] ?? '#C9A84C') : '#9994A8',
            }}>
              {p === 'all' ? 'All' : p}
            </button>
          ))}
      ...
      {visible.length === 0 && (
        <div style={{ textAlign: 'center', padding: '64px 24px', background: '#0F0F1A',
          borderRadius: 14, border: '1px solid #1E1E30' }}>
          <p style={{ fontSize: 18, fontWeight: 600, color: '#E8E4D8', marginBottom: 8 }}>No keys yet</p>
          <p style={{ color: '#5A5570', fontSize: 14, marginBottom: 24, maxWidth: 320, margin: '0 auto 24px' }}>
            Hit Quick test key to generate keys for each persona instantly.
          </p>
          <button style={{ ...goldBtn, background: '#52C47A' }}
            onClick={() => setModal({ type: 'quick' })}>
            Generate first key
          </button>
        </div>
      ...
      {Array.from(new Set(visible.map(k => k.persona))).map(persona => (
        <div key={persona} style={{ marginBottom: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: personaColors[persona] ?? '#C9A84C' }}>
              {persona}
            </span>
            <span style={{ fontSize: 11, color: '#5A5570' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
            {visible.filter(k => k.persona === persona).map(key => (
              <KeyRow key={key.keyId} k={key}
                accentColor={personaColors[persona] ?? '#C9A84C'}
