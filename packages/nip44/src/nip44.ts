...
// NIP-44 v2 -- encrypted payload for one peer, vendored byte-identically
// from tapit-wallet's tapit-attest/src/core/nip44.ts (Cut B stage B3,
// docs/integration-phase1-signin-and-bridge.md) so DynastyTrust can
// encrypt a psbt-cosign request to a Tapit member's pubkey before
// publishing it over @dynastytrust/nostr-transport. Same
// minimal-dependency + parity-test discipline as that package and
// @dynastytrust/bip341-psbt-signer (B0): this module is vendored
// byte-identically into every repo that needs it, and test/parity.test.mjs
// asserts a fixed golden vector so a drifted copy fails there and only
// there. To change this module: edit it in tapit-wallet (the canonical
// source), run its own nip44 tests, then mirror src/, test/, package.json,
// and tsconfig.json byte-for-byte here -- never hand-edit this copy.
//
